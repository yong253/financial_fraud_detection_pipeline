...
PaySim 기반 금융 거래 **엔드투엔드 배치 데이터 파이프라인**. 포트폴리오 프로젝트.

**핵심:** 금융 데이터를 단 1건도 유실·중복 없이(무손실·무중복·멱등) Bronze→Silver→Gold로 처리하고,
레이어 간 정합성을 매 실행 자동 검증하는 것이 목표. 원본의 사기 라벨(`isFraud`)·기존 룰 탐지
(`isFlaggedFraud`)는 그대로 보존하여, 거래 현황·기존 룰의 탐지 성능·미탐지 사기(`isFraud=1 &
isFlaggedFraud=0`)를 파이프라인이 집계·가시화한다. (우리가 새 탐지기를 만드는 것이 아니라, 기존
룰의 성능을 신뢰성 있게 분석하는 것 — 사기 탐지 로직 자체는 스코프 아웃.)
...
|------|------|
| 메시지 큐 | Apache Kafka |
| 수집 | Kafka Connect (GCS Sink) |
| 배치 | Apache Spark (Dataproc Serverless) |
...
| 디렉토리 | 내용 |
|------|------|
| `kafka/CLAUDE.md` | 수집 흐름(CSV→Kafka→Kafka Connect→Bronze) |
| `spark/CLAUDE.md` | Silver 레이어 상세(변환·품질검증·Quarantine·무손실/무중복·is_suspicious 집계) |
...
- **Silver (GCS→BigQuery)** — 품질검증 + Quarantine(무손실) + dedup(무중복) + `is_suspicious`
  플래그(원본 라벨 파생: 기존 룰 미탐지 사기). 상세: `spark/CLAUDE.md`.
- **Gold (BigQuery — DBT)** — `hourly_summary`/`undetected_fraud`(기존 룰 미탐지 사기 집계)
  /`account_risk`. 상세: `dbt/CLAUDE.md`.
